...
function StoryThumbnail({ story }) {
  if (story.mediaUrl || story.thumbnailUrl) {
    return (
      <div className="w-10 h-14 rounded-xl overflow-hidden shrink-0 border border-gray-100 shadow-sm bg-gray-50">
        <img
  ...
  return (
    <div className="w-10 h-14 rounded-xl shrink-0 border border-gray-100 shadow-sm bg-gradient-to-br from-indigo-50 to-purple-50 flex items-center justify-center">
      {isVideo ? (
...
function MetricPill({ label, value, highlight = false }) {
  return (
    <div className="flex flex-col items-center px-3 py-1.5 rounded-xl bg-gray-50 min-w-[64px]">
      <span
        className={`text-sm font-black ${
          highlight ? "text-indigo-600" : "text-[#0A0A0A]"
        }`}
      >
        {value}
      </span>
...
function EmptyStories() {
  return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-50 to-purple-100 flex items-center justify-center">
        <ImageIcon size={22} className="text-indigo-300" />
      </div>
      <div className="text-center">
        <p className="text-sm font-bold text-gray-400">No stories found</p>
        <p className="text-xs text-gray-300 mt-1">Stories will appear here after syncing</p>
      </div>
    </div>
  );
}
...
function StoriesSkeleton() {
  return (
    <div className="space-y-3 animate-pulse">
      {[1, 2, 3].map((n) => (
        <div key={n} className="flex items-center gap-4 p-4 rounded-2xl border border-gray-50">
          <div className="w-10 h-14 rounded-xl bg-gray-100 shrink-0" />
          ...
          <div className="flex gap-2">
            {[1, 2, 3, 4].map((x) => (
              <div key={x} className="w-14 h-10 bg-gray-50 rounded-xl" />
            ))}
          </div>
        </div>
      ))}
              ...
              <thead className="bg-gray-50/50">
                <tr>
                  {[
                    ...
                    <th
                      key={i}
                      className={`px-5 py-4 text-[9px] font-bold text-gray-400 uppercase tracking-widest ${
                    ...
                    <td className="px-5 py-3 text-center">
                      <span
                        className={`text-xs font-bold ${
                          story.exitRate > 0.5 ? "text-red-500" : "text-gray-500"
                        }`}
                      >
            ...
            >
              <span
                className={`text-xl font-black ${
                  item.highlight ? "text-indigo-600" : "text-gray-800"
                }`}
              >
                {item.value}
              </span>
