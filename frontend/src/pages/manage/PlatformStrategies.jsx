      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden text-left font-sans max-w-full">
        ...
        <div className="p-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <img src={brandLogo} alt="Avatar" className="w-10 h-10 rounded-full object-cover border border-gray-100" />
            <div>
              <div className="text-sm font-semibold text-gray-900 hover:underline cursor-pointer">{brandName}</div>
              <div className="text-xs text-gray-500 flex items-center gap-1">
                {scheduledTimeStr} · <span className="inline-block w-3 h-3 bg-gray-500 rounded-full text-[8px] text-white text-center leading-3 font-black">🌐</span>
              </div>
            </div>
          </div>
        ...
        {/* Actions Bar Mockup */}
        <div className="px-4 py-2.5 border-t border-gray-150 flex items-center justify-around text-xs text-gray-500 font-semibold bg-[#FAFAFA]">
          <span className="cursor-pointer hover:bg-gray-100 px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-all">👍 Like</span>
          <span className="cursor-pointer hover:bg-gray-100 px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-all">💬 Comment</span>
          <span className="cursor-pointer hover:bg-gray-100 px-3 py-1.5 rounded-md flex items-center gap-1.5 transition-all">➡️ Share</span>
        </div>
      </div>
    );
  }
...
const InstagramStrategy = {
  ...
  renderPreview: (post, brand, postImage) => {
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden text-left font-sans max-w-[420px] mx-auto">
        {/* Header */}
        <div className="p-3 flex items-center justify-between border-b border-gray-100">
          <div className="flex items-center gap-3">
            <img src={brandLogo} alt="Avatar" className="w-8 h-8 rounded-full object-cover border border-gray-100" />
            <span className="text-xs font-bold text-gray-900 hover:underline cursor-pointer">{instaUsername}</span>
          </div>
          <span className="text-gray-400 font-bold hover:text-gray-600 cursor-pointer">•••</span>
        </div>
        ...
        <div className="aspect-square bg-gray-50 flex items-center justify-center">
          {postImage ? (
            ...
            <span className="text-gray-400 text-xs font-semibold">Image Container (1:1)</span>
          )}
        </div>

        {/* Action Icons */}
        <div className="p-3 flex justify-between items-center text-xl text-gray-800">
        ...
        <div className="px-3 pb-4 text-xs">
          <div className="font-bold text-gray-900 mb-1">Liked by metricool and others</div>
          <p className="leading-relaxed">
            <span className="font-bold mr-1.5 text-gray-900">{instaUsername}</span>
            <span className="text-gray-700 whitespace-pre-wrap">{caption}</span>
          </p>
        </div>
      </div>
    );
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden text-left font-sans max-w-full">
        ...
        <div className="aspect-video bg-black relative flex items-center justify-center group cursor-pointer">
          {postImage ? (
            ...
            <span className="text-gray-500 text-xs">Video Player Container (16:9)</span>
          )}
        ...
        <div className="p-4 flex gap-3">
          <img src={brandLogo} alt="Channel" className="w-9 h-9 rounded-full object-cover border border-gray-100 shrink-0" />
          <div>
            <h4 className="text-sm font-semibold text-gray-900 line-clamp-2 leading-snug">{post?.title || "Untitled Video"}</h4>
            <div className="text-[11px] text-gray-500 mt-1 flex flex-wrap items-center gap-1.5">
...
const TiktokStrategy = {
  ...
  name: "TikTok",
  renderIcon: (size) => (
    <span className="w-7 h-7 rounded-full bg-gray-50 flex items-center justify-center text-black">
      <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
      ...
      <div className="bg-black border border-gray-800 rounded-2xl shadow-lg overflow-hidden text-left font-sans max-w-[320px] mx-auto aspect-[9/16] relative flex flex-col justify-end text-white">
        ...
        {postImage ? (
          ...
          <div className="absolute inset-0 bg-gradient-to-b from-gray-900 to-black flex items-center justify-center">
            <span className="text-gray-600 text-xs">Vertical Video Container (9:16)</span>
          </div>
        )}
...
const XStrategy = {
  ...
  renderPreview: (post, brand, postImage, scheduledTimeStr) => {
    ...
    return (
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm p-4 text-left font-sans max-w-full">
        {/* Header */}
        <div className="flex gap-3">
          <img src={brandLogo} alt="Avatar" className="w-10 h-10 rounded-full object-cover border border-gray-100 shrink-0" />
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1 min-w-0">
                <span className="text-sm font-bold text-gray-900 truncate hover:underline cursor-pointer">{brandName}</span>
                <span className="text-xs text-gray-500 truncate">@{handle}</span>
                <span className="text-xs text-gray-400">·</span>
                <span className="text-xs text-gray-500">{scheduledTimeStr}</span>
              </div>
              <span className="text-gray-400 font-bold hover:text-gray-600 cursor-pointer">•••</span>
            </div>

            {/* Caption */}
            <p className="text-sm text-gray-900 mt-1 leading-relaxed whitespace-pre-wrap">{caption}</p>
            ...
            {/* X Actions icons bar */}
            <div className="mt-4 flex justify-between text-xs text-gray-500 max-w-[85%] font-medium">
...
const FallbackStrategy = {
  ...
  renderIcon: (size, platform) => (
    <span className="w-7 h-7 rounded-full bg-gray-100 text-[10px] font-bold text-gray-500 flex items-center justify-center uppercase">
      {platform?.slice(0, 2) || "?"}
    </span>
  ),
  renderPreview: (post) => {
    const caption = post?.caption || "";
    return (
      <div className="bg-white p-6 border border-gray-200 rounded-xl text-center shadow-sm">
        <h4 className="text-sm font-bold text-gray-700">Preview not available for this network</h4>
        <p className="text-xs text-gray-500 mt-1 whitespace-pre-wrap">{caption}</p>
      </div>
    );
  }
};
