          ...
          <div>
            ...
            <div className="flex gap-3">
              {["X", "in", "YT", "IG"].map((s) => (
                <button
                  key={s}
                  className="w-8 h-8 rounded-md flex items-center justify-center hover:text-white transition-colors duration-150"
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            {["Privacy Policy", "Terms of Service", "Cookie Settings"].map((link) => (
              <button key={link} className="hover:text-white transition-colors duration-150" style={{ fontSize: 11, color: "#555" }}>
                {link}
              </button>
            ))}
            <button
