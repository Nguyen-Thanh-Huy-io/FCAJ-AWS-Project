      ...
      <div style={{ maxWidth: 480, width: "100%", textAlign: "center" }}>
        ...
        {type === "404" && (
          <>
            ...
            <div className="flex gap-3 justify-center mb-8">
              <button onClick={() => window.history.back()} className="px-5 py-2.5 rounded-lg hover:bg-[#F8F8F7] transition-colors" style={{ fontSize: 14, border: "0.5px solid #E5E7EB", color: "#0A0A0A" }}>
                ← Go Back
              </button>
              <button onClick={() => onNavigate?.("dashboard")} className="px-5 py-2.5 rounded-lg bg-[#0A0A0A] text-white hover:bg-[#1E1E1E] transition-colors" style={{ fontSize: 14 }}>
                Go to Dashboard
              </button>
            </div>
            <div>
              ...
              <div className="space-y-2">
                {["Content Planner", "Analytics", "Stream Schedule", "Team Settings"].map((link) => (
                  <button key={link} onClick={() => onNavigate?.(link.toLowerCase().replace(" ", "-"))} className="block mx-auto hover:underline" style={{ fontSize: 13, color: "#0A0A0A" }}>
                    → {link}
                  </button>
                ))}
              </div>
        ...
        {type === "500" && (
          <>
            ...
            <div className="flex gap-3 justify-center mb-6">
              <button onClick={() => window.location.reload()} className="px-5 py-2.5 rounded-lg bg-[#0A0A0A] text-white" style={{ fontSize: 14 }}>
                Refresh Page
              </button>
              <button className="px-5 py-2.5 rounded-lg" style={{ fontSize: 14, border: "0.5px solid #E5E7EB", color: "#0A0A0A" }}>
                Check Status Page ↗
              </button>
            </div>
        ...
        {type === "offline" && (
          <>
            ...
            <button className="px-5 py-2.5 rounded-lg bg-[#0A0A0A] text-white mb-6" style={{ fontSize: 14 }}>
              Try Again
            </button>
            <div className="flex items-center justify-center gap-2">
...
export function ErrorPages({ onNavigate }: { onNavigate?: (page: string) => void }) {
  return (
    <div className="p-6 space-y-8" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="grid grid-cols-3 gap-4">
        {(["404", "500", "offline"] as const).map((type) => (
          <div key={type} className="bg-white rounded-xl overflow-hidden" style={{ border: "0.5px solid #E5E7EB", height: 400 }}>
            ...
            <ErrorPage type={type} onNavigate={onNavigate} />
          </div>
        ))}
      </div>
    </div>
