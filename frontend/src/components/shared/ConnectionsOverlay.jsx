  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/40 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="w-full max-w-6xl h-[90vh] bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col relative animate-in zoom-in-95 duration-300">
        ...
        {/* Brand Selector Bar */}
        <div className="px-10 py-4 bg-[#F8F9FB] border-b border-gray-100 flex items-center justify-between shrink-0">
           <div className="flex items-center gap-3">
              <span className="text-xs font-bold text-gray-400 uppercase tracking-widest">Active Brand:</span>
              {loadingBrands ? (
                ...
                <div className="relative" ref={dropdownRef}>
                  <button 
                    ...
                    className="bg-white border border-gray-200 rounded-xl px-4 py-2 flex items-center gap-3 shadow-sm hover:border-gray-300 transition-all font-bold text-sm text-[#0A0A0A] cursor-pointer"
                  >
                  ...
                  {isDropdownOpen && (
                    <div className="absolute top-full left-0 mt-1 w-64 bg-white border border-gray-200 rounded-xl shadow-xl z-[2010] overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
                      {brands.map((brand) => (
                        <button 
                          key={brand.id}
                          onClick={() => { setSelectedBrand(brand); setIsDropdownOpen(false); }}
                          className={`w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-[#F3F4F6] transition-colors cursor-pointer border-none ${selectedBrand.id === brand.id ? 'bg-[#F9FAF8]' : 'bg-white'}`}
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-xs font-bold text-[#0A0A0A] truncate">{brand.name}</div>
                ...
                <div className="text-xs text-gray-400 italic">No brands found.</div>
              )}
           </div>
