      ...
      <div className="flex items-start gap-4">
        ...
        <div className="relative shrink-0 w-12 h-12">
          {conv.participants && conv.participants.length > 1 ? (
            <>
              <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-white shadow-sm bg-gray-50 absolute top-0 left-0 z-10">
              ...
              <div className="w-9 h-9 rounded-full overflow-hidden border-2 border-white shadow-sm bg-[#4A3AFF] absolute bottom-0 right-0 z-0 flex items-center justify-center text-[10px] font-bold text-white">
                {conv.participants[1].avatar ? (
                  <img src={conv.participants[1].avatar} alt="" className="w-full h-full object-cover" />
                ) : conv.participants[1].name.charAt(0)}
              </div>
            </>
          ) : (
            <div className="w-12 h-12 rounded-full overflow-hidden border border-gray-100 bg-gray-50">
              {conv.avatar && conv.avatar.length > 2 ? (
                ...
                <div className="w-full h-full flex items-center justify-center font-bold text-gray-400">{conv.user.charAt(0)}</div>
              )}
            </div>
          )}
