      ...
      <div className="p-5 space-y-4">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-gray-400 font-medium">{parseInt(videoContext.subscriberCount || 0).toLocaleString()} subscribers</span>
            </div>
          </div>
          <div className="flex gap-2">
            <div className="px-3 py-1 bg-gray-100 rounded-full text-[10px] font-bold text-gray-700">Join</div>
            <div className="px-3 py-1 bg-black text-white rounded-full text-[10px] font-bold">Subscribe</div>
          </div>
        </div>
        <div className="flex items-center gap-4 border-t border-gray-50 pt-4 overflow-x-auto no-scrollbar">
          {[
            ...
            { icon: <Scissors size={14}/>, label: "Clip" }
          ].map((btn, idx) => (
            <div key={idx} className="flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 rounded-full text-gray-500 whitespace-nowrap">
              {btn.icon}
              <span className="text-[10px] font-bold">{btn.label}</span>
            </div>
          ))}
          <div className="p-1.5 bg-gray-50 rounded-full text-gray-500"><MoreHorizontal size={14}/></div>
        </div>
      </div>
      <div className="px-5 py-3 bg-[#F3F4F6]/50 border-t border-gray-50">
        <p className="text-[11px] text-gray-600 font-medium">
          {parseInt(videoContext.viewCount || 0).toLocaleString()} views, {new Date(videoContext.publishedAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
        </p>
        <span className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">...more</span>
      </div>
    </a>
  );
};
