...
Tài liệu này mô tả chi tiết kiến trúc tích hợp kênh TikTok, cách hệ thống đồng bộ dữ liệu số liệu phân tích (Analytics) thực tế thông qua mô hình ủy quyền (Delegation Pattern), giải pháp khắc phục các lỗi hiển thị chỉ số, đồng bộ thời gian (Date Filter) và nâng cấp cơ sở dữ liệu để đồng bộ thông tin của Facebook.
...
Hệ thống chuyển giao toàn bộ logic phân tích từ Mock sang dữ liệu thực tế thông qua việc ủy quyền cuộc gọi dịch vụ từ `TikTokService` sang `TikTokAnalyticsService`.
...
Để khắc phục hiện tượng dữ liệu không thay đổi khi chọn bộ lọc ngày, frontend áp dụng bộ lọc client-side trên cả tab **POSTS** và **COMMUNITY** giúp tăng tốc độ phản hồi và tiết kiệm quota gọi API.
...
* **Triệu chứng**: Giao diện các con số thống kê và biểu đồ của TikTok đều hiển thị `0`.
* **Nguyên nhân**: Trong `usePlatformDashboard.js`, sự kiện gọi hàm tải bài đăng (`fetchPublishedVideos`) chỉ bắt các tab của YouTube/Facebook (`published`, `posts_list`) mà bỏ qua tab `"posts"` của TikTok.
* **Khắc phục**: Thêm điều kiện `(activeTab === "posts" && platform === "tiktok")` để tự động tải danh sách video thật khi người dùng nhấn sang tab bài viết của TikTok.

### 2. Lỗi dữ liệu cộng đồng (Community Metrics) của TikTok bị ảo
* **Triệu chứng**: Biểu đồ tăng trưởng người theo dõi hiển thị 5 bài đăng ngẫu nhiên và 47 follower mới wavy mặc dù tài khoản thực tế chỉ có 2 bài đăng và 1 follower.
* **Nguyên nhân**: `TikTokService` sử dụng hàm mock ngẫu nhiên `generateMockTikTokAnalytics` thay vì sử dụng dữ liệu tính toán từ video thực của `TikTokAnalyticsService`.
* **Khắc phục**: Chuyển giao toàn bộ việc đồng bộ số liệu và tạo báo cáo phân tích sang `TikTokAnalyticsService` để lấy thông tin thực tế từ TikTok Developer APIs.

### 3. Lỗi bộ lọc ngày (Date Filter) không hoạt động
* **Triệu chứng**: Thay đổi ngày bắt đầu/kết thúc trên thanh công cụ nhưng biểu đồ và số liệu tổng hợp không thay đổi.
* **Khắc phục**: 
  - Truyền `dateRange` xuống các tab con `TikTokPostsTab` và `TikTokCommunityTab`.
  - Sử dụng `React.useMemo` để lọc dữ liệu cục bộ theo khoảng ngày đã chọn, đảm bảo biểu đồ mượt mà và cập nhật tức thì khi người dùng tương tác.

### 4. Lỗi tràn giới hạn ký tự cột JSON của Facebook (`audienceDemographicsJson`)
* **Triệu chứng**: MySQL trả về lỗi: *The provided value for the column is too long for the column's type. Column: audienceDemographicsJson*.
* **Nguyên nhân**: Dữ liệu nhân khẩu học lớn vượt quá giới hạn 64KB của kiểu dữ liệu `TEXT` mặc định trong Prisma.
* **Khắc phục**: Thay đổi định nghĩa cột trong [schema.prisma](file:///d:/Fullit/projects/PubliCast/backend/prisma/schema.prisma) sang `@db.LongText` (hỗ trợ tới 4GB) và thực hiện `npx prisma db push`.
