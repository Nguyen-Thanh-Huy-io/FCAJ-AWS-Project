...
Hệ thống hỗ trợ song song hai cơ chế upload để đảm bảo tính linh hoạt và dự phòng:

### 1. Cơ chế Tải lên Trực tiếp (Direct Upload - Khuyên dùng)
- **Luồng hoạt động**:
...
### 2. Cơ chế Tải lên qua Server (Traditional Server-side Upload - Dự phòng)
- **Luồng hoạt động**: Gọi qua endpoint `/api/media/upload` truyền thống. Server đóng vai trò nhận file buffer thông qua middleware (`multer`), sau đó tự động đẩy lên Cloudinary cloud trước khi lưu trữ bản ghi vào MySQL.
- **Lưu ý**: Chỉ sử dụng cho các tệp tin dung lượng nhỏ hoặc làm phương án dự phòng khi các chính sách bảo mật client-side chặn kết nối trực tiếp tới Cloudinary.

### 3. Tải lên bù (Resumable Upload - Cấp độ 2)
...
| `POST` | `/api/media/upload` | Tải file trực tiếp qua trung gian Server (Dự phòng). |
...
---
*Tài liệu được cập nhật ngày 03/06/2026.*
